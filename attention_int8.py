...
def smooth_mean_d(Nd: torch.Tensor):
    mean_d =  Nd.mean(dim=-1)
    return Nd - mean_d
    

def quantized_by_token_block(
    bh_td:torch.Tensor, 
    ...
    Convert [BH, N, d] tensor into N // token_block_size
    [num_blocks, block_size, d] 

    Please add zero padding before convertion

    quantize each block_t =  [block_size, d] 
    with mC_int8 = max( abs(chunk_tensor) ) / 127
    ...
    Returns:
        b_sd_int8: quantized int8 tensor
        delta_b: shape [num_blocks] multiplier to revert back to fp16

    """

    BH, T, d = bh_td.shape
    C = token_chunk_size # chunk size
    N = T // C # number of chunk

    bh_ncd =  bh_td.reshape(BH, N, C, d)
    max_bh_n = bh_ncd.abs().max(dim=[1,2], keepdim=True)
    max_bh_n = max_bh_n  / 127  

    bh_ncd = bh_ncd / max_bh_n
    bh_ncd_int8 = bh_ncd.to(torch.int8)

    return bh_ncd_int8, max_bh_n
...
def helion_atten_int8_online_fwd(
...
def helion_atten_int8_online_bwd(
    q_input: torch.Tensor,
    k_input: torch.Tensor,
